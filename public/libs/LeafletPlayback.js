  ...
  statics: {
    ...
    ParseGPX: function(gpx) {
      ...
      for (let trackIdx = 0; trackIdx < trks.length; trackIdx++) {
        ...
        geojson.properties.trk.name = track.querySelector('name')?.textContent || '';
        geojson.properties.trk.desc = track.querySelector('desc')?.textContent || '';
        geojson.properties.trk.type = track.querySelector('type')?.textContent || '';
        geojson.properties.trk.src = track.querySelector('src')?.textContent || '';
        ...
        for (let i = 0; i < pts.length; i++) {
          ...
          const timeStr = p.querySelector('time')?.textContent || '';
          const eleStr = p.querySelector('ele')?.textContent || '0';
